...
/**
 * Tenta extrair gameId/productId/planId de um "product_hash" tipo:
 *   fivem_fivem-external_month
 */
function parseProductHash(productHash: any) {
  const raw = String(productHash || '')
  if (!raw) return { gameId: '', productId: '', planId: '' }

  // aceita "_" ou ":" como separador
  const s = raw.includes(':') ? raw.split(':').join('_') : raw
  const parts = s.split('_').filter(Boolean)
  if (parts.length >= 3) {
    const gameId = parts[0]
    const planId = parts[parts.length - 1]
    const productId = parts.slice(1, -1).join('_')
    return { gameId, productId, planId }
  }
  return { gameId: '', productId: raw, planId: '' }
}
function planLabelFromId(planId: string) {
  const p = String(planId || '').toLowerCase()
  if (p === 'day') return '1 Day'
  if (p === 'week') return '1 Week'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // ✅ exige login (você já trava add cart sem login)
    ...
    // ===================== SALVA ORDER PENDENTE NO BANCO =====================
    if (!normalized.transaction_hash) {
      return NextResponse.json({ error: 'A TriboPay não retornou transaction_hash.' }, { status: 502 })
    }

    await dbConnect()

    // cria snapshot do pedido pra aparecer no dashboard
    await Order.updateOne(
      { transactionHash: String(normalized.transaction_hash) },
      {
        $setOnInsert: {
          userId: user.uid,
          ...
          items: (items || []).map((it: any) => {
            const ph = parseProductHash(it?.product_hash || it?.hash || '')
            return {
              gameId: String(it?.gameId || ph.gameId || ''),
              productId: String(it?.productId || it?.id || ph.productId || ''),
              title: String(it?.title || it?.name || 'Produto'),
              cover: String(it?.cover || ''),
              tag: String(it?.tag || ''),
              planId: String(it?.planId || ph.planId || ''),
              planLabel: String(it?.planLabel || planLabelFromId(it?.planId || ph.planId || '')),
    ...
    // ✅ resposta pro front: SOMENTE o necessário
    return NextResponse.json(
      {
